...
def render_weather(ax: Axes, lat=None, lon=None, title=None, show_xlabel=True):
    ...
    if not data:
        ax.text(0.5, 0.5, "Weather data unavailable",
                ha='center', va='center',
    ...
    for period in periods:
        ...
        is_snowy = any(keyword in forecast for keyword in
                      ['snow', 'flurries', 'sleet', 'wintry', 'freezing'])
    ...
    ax.set_ylabel('°F', fontsize=config.FONT_SIZE_SMALL, color=color_temp, rotation=0,
                  labelpad=10, ha='right', va='center')
    ...
    color_precip = '#666666'
    ax2.plot(range(len(precip_probs)), precip_probs, linewidth=1.5, color=color_precip,
    ...
    if snow_indices:
        ...
        ax2.scatter(snow_indices, snow_precip_values, marker='*', s=30,
                   color=color_precip, alpha=0.8, zorder=5)
    ...
    ax.set_title(f"{display_title} - {current_temp}°F, {current_desc}",
                 fontsize=config.FONT_SIZE_BODY, pad=5)
